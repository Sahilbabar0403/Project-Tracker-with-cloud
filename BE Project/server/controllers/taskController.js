...
export const getTasks = async (req, res) => {
  try {
    ...
    let query = { isTrashed: isTrashed === "true" }; // Ensure proper boolean check

    // Restrict non-admin users to only see tasks where they are assigned
    if (!isAdmin) {
      query.team = { $in: [userId] }; // Correct way to filter tasks where user is in team array
    }
